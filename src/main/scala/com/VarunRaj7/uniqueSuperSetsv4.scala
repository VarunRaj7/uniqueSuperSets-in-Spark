...
object uniqueSuperSetsv4 {
     ...
     * 
     */
    class ItemHash(val is:immutable.TreeSet[Int], val uId:Long, val iss:Int)
    extends Ordered[ItemHash] with Serializable           // Ordered trait is required to compare Items 
    {
      ...
      def --(i:immutable.TreeSet[Int]):ItemHash={
        ...
        return new ItemHash(temp_is, uId, iss)
      }
      ...
      def compare(that:ItemHash):Int = {
        ...
        if (this.iss>that.iss) {
          ...
          this.uId.toString.compareTo(that.uId.toString)
        }
      }
   ...
   def mapperFunc(elem:Tuple2[Long, String])={//:Set[Tuple2[Int, ItemHash]]={  
       
      // Create a immutable.TreeSet[Int] with items given in String
      val elem_set = new immutable.TreeSet[Int]()++(elem._2.split(" ").map(_.toInt))
      
      // Create a HashCode for the items given in String
      val elemIH = new ItemHash(elem_set, elem._1, elem_set.size)
   ...
   def main(args: Array[String]) {
    ...
    // Creating a SparkSession
     ...
     // Given uId?
     val given_uId = args(3).toBoolean
     ...
     var rdd_s = sc.emptyRDD[Tuple2[Long,String]]
     if(given_uId){
       rdd_s = sc.textFile(args(4), map_partitions)
                 .map(elem => { val elem_split = elem.stripSuffix("\n").split(",") 
                                (elem_split(0).toLong, elem_split(1))})//.replace(",", " ").distinct()  
     }
     else{
       rdd_s = sc.textFile(args(4), map_partitions)
                 .map(elem => elem.stripSuffix("\n"))
                 .zipWithUniqueId()
                 .map({case(str,uId) => (uId, str)})//.replace(",", " ").distinct()
     }
     ...
     var rdd_s_df = rdd_s.toDF(Seq("uId", "Items"):_*)
                         .cache()
     ...
     */
     ...
     val rdd_s3 = rdd_s1.aggregateByKey(mutable.TreeSet[ItemHash]())(seqOp, combOp) // aggregateByKey Operation with seqOp and combOp
                        .map(elem => for(x<-elem._2) yield x.uId)  // mapper function to seek out all the hash values given by each key
     ...
     */
     ...
     val unique_df_hv = rdd_s3.toDF("uId").dropDuplicates()
     ...
     val res_df = rdd_s_df.join(unique_df_hv, Seq("uId"),"inner").repartition(final_res_partitions)
     
     println("Unique Super Sets count: ",res_df.count)
     
     val res_df_items = res_df.drop("uId") 
     
     res_df_items.write.format("csv").save(args(4))
     //res_df_items.explain()
     //res_df_items.show()
   }  
}