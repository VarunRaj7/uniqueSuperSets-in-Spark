...
object uniqueSuperSets2 {
     ...
     * 
     */
    class ItemHash(val is:immutable.TreeSet[Int], val acts:String, val iss:Int)
    extends Ordered[ItemHash] with Serializable           // Ordered trait is required to compare Items 
    {
      ...
      def --(i:immutable.TreeSet[Int]):ItemHash={
        ...
        return new ItemHash(temp_is, acts, iss)
      }
      ...
      def compare(that:ItemHash):Int = {
        ...
        if (this.iss>that.iss) {
          ...
          this.acts.toString.compareTo(that.acts.toString)
        }
      }
   ...
   def mapperFunc(elem:String)={//:Set[Tuple2[Int, ItemHash]]={  
      ...
      val elemIH = new ItemHash(elem_set, elem_set.mkString(" "), elem_set.size)
   ...
   def main(args: Array[String]) {
    ...
    // Creating a SparkSession
     ...
     var rdd_s_df = rdd_s.map(elem => ((new immutable.TreeSet[Int]()++(elem.split(" ").map(_.toInt))).mkString(" ")))
                         .toDF(Seq("Items"):_*)
                         .cache()
     ...
     */
     ...
     val rdd_s3 = rdd_s1.aggregateByKey(mutable.TreeSet[ItemHash]())(seqOp, combOp) // aggregateByKey Operation with seqOp and combOp
                        .map(elem => for(x<-elem._2) yield x.acts)  // mapper function to seek out all the hash values given by each key
